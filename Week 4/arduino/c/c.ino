...
#define btn3 A2             // Definisi pin button
String c = "MATI";          // Pesan yang akan di kirim
...
void setup() {
  ...
  pinMode(btn3, INPUT_PULLUP);  // set button sebagai input
}
...
void loop() {
  ...
  lcd.setCursor(0,0);
  lcd.print("KIRIM");           // Menuliskan "KIRIM" di 
  ...
  int n3= digitalRead(btn3);    // Membaca status button (HIGH/LOW)
  ...
  if (n3==0) {
    ...
    Serial.print (c);       // Menampilkan pesan ke virtual terminnal
    lcd.setCursor(0,1);     // Menempatkan cursor pada alamat array(0,1) 
    lcd.print(c);           // Menampilkan pesan di LCD
